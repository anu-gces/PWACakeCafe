...
import { Route as editMenuRoute, MenuCategory } from "@/routes/home/editMenu";
  ...
  return (
    <div className="">
      <div className="w-full">
        ...
        <CreateOrderDrawer addToCart={addToCart} />
...
function CreateOrderDrawer({ addToCart }: { addToCart: CartItem[] }) {
  return (
    <Drawer>
...
function Bill({ addToCart }: { addToCart: CartItem[] }) {
  const [items, setItems] = useState(addToCart);

  const updateQty = (foodId: string, newQty: number) => {
    setItems((prev) => prev.map((item) => (item.foodId === foodId ? { ...item, qty: newQty } : item)));
  };

  const [discountRate, setDiscountRate] = useState(0); // default 10%
  const [taxRate, setTaxRate] = useState(0); // default 13%

  const subtotal = items.reduce((sum, item) => sum + item.foodPrice * item.qty, 0);
              ...
              <TableRow key={item.foodId}>
                ...
                <TableCell className="text-center">
                  <Input
                    ...
                    min={0}
                    value={item.qty}
                    onChange={(e) => updateQty(item.foodId, parseInt(e.target.value || "0"))}
